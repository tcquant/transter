...
def shifting(dt,Option,Option_,io,id,selling,buying,type_,capital,rollover,initial_capital,obj,execs,trades,row,file,net_pnl):
...
def generate_trades_syn(eh,em,id,futures, ce, pe, gen_call, gen_put, io=0, buying='open', selling='close', DeltaLB=0.2,delta_range=0.3, DeltaUB = 0.5,initial_capital=1000, sharpie_per_trade_index='open'):

    trades,call_iv,put_iv,execs,running_pnl, net_pnl,capital, rollover, streak,iv,civ = [],[],[], [], [], [0], initial_capital, 0, 0, [],[]
    put = {'pos': 0,'sell_id':0, 'entry_time': None, 'strike': None, 'entry_price': 0,'iv':[],
        ...
        'pnl': 0, 'type': None, 'pnl_sum': 0, 'transaction_cost': 0, 'slippage': 0, 'logs': [],'sharpie_ratio': 0, 'drawdown': 0,
    ...
    with open(f"_Logs.txt", "w") as file:
        for i in range(1, len(futures) - 1):
            ...
            delta_put.append((dt,put['entry_delta'])); delta_call.append((dt,call['entry_delta'])); positions.append((dt, put['pos']))
            print("\n", file=file);    print("`",cc,"`", file=file);     print("Time", dt, f"Spot = {row['close']}", file=file);        cc+=1
            c_iv,p_iv = gen_call[(gen_call['date_timestamp']==dt)] , gen_put[(gen_put['date_timestamp']==dt)]
            civ_ind,piv_ind = np.argmin(np.abs(c_iv['strike']-c_iv['syn_fut'])) , np.argmin(np.abs(p_iv['strike']-p_iv['syn_fut']))
            ...
            if streak >= 1 and dt.time() <= time(15, 00, 0):
                ...
                if put['pos'] == 0 and call['pos'] == 0:
                    ...
                    if Cindex == -1:
                        print(f"No Call Available in the range of [{DeltaLB - delta_range}, {DeltaLB + delta_range}] at Timestamp = {dt}",file=file)
                    else:
                        Pindex,Put_Option = GetallOptionsClosetoDelta(gen_put, Call_Option['delta'], delta_range/2, dt,'PE',file)
                        if Pindex == -1:
                            print(f"No Put Available in the range of [{-Call_Option['delta'] - delta_range}, {-Call_Option['delta'] + delta_range}] (near to call option delta) at Timestamp = {dt}",file=file)
                        else:
                            print(f"entry with call strike {Call_Option['strike']} and put strike = {Put_Option['strike']} at dt = {dt}",file=file)
                            capital, rollover = sell(Call_Option, selling, io, id, call,row, 'CE', capital, rollover,initial_capital)
                            execs.append({
                                'id': id, 'time': dt, 'opt_type': 'CE', 'log': f"Initial Entry", 'delta':Call_Option['delta'], 'strike': Call_Option['strike'],
                            ...
                            capital, rollover = sell(Put_Option, selling, io, id, put, row, 'PE', capital, rollover,initial_capital)
                            execs.append({
            ...
            if call['pos'] == -1 and put['pos'] == -1:
                ...
                if not gen_call[(gen_call['date_timestamp'] == dt) & (gen_call['strike'] == call['strike'])].empty and not gen_put[(gen_put['date_timestamp'] == dt) & (gen_put['strike'] == put['strike'])].empty:

                    Call_Option = gen_call[(gen_call['date_timestamp'] == dt) & (gen_call['strike'] == call['strike'])].iloc[0]
                    Put_Option = gen_put[(gen_put['date_timestamp'] == dt) & (gen_put['strike'] == put['strike'])].iloc[0]

                    print("Current Deltas: " ,Call_Option['delta'] , Put_Option['delta'],round(Call_Option['delta']+Put_Option['delta'],3),file=file)

                    if abs(Call_Option['delta'] + Put_Option['delta']) > delta_range:
                        if (Call_Option['delta'] > -Put_Option['delta'] and Call_Option['delta'] <= DeltaUB) or (-Put_Option['delta'] > Call_Option['delta'] and -Put_Option['delta'] >= DeltaUB):
                        ...
                        elif (-Put_Option['delta'] > Call_Option['delta'] and -Put_Option['delta'] <= DeltaUB) or (Call_Option['delta'] > -Put_Option['delta'] and Call_Option['delta'] >= DeltaUB):
                    ...
                    if gen_call[(gen_call['date_timestamp'] == dt) & (gen_call['strike'] == call['strike'])].empty:
                        print(f"No Availability of call strike = {call['strike']} at dt = {dt} for Delta Shifting",file=file)
                    if gen_put[(gen_put['date_timestamp'] == dt) & (gen_put['strike'] == put['strike'])].empty:
                        print(f"No Availability of put with strike {put['strike']} at dt = {dt} for Delta Shifting",file=file)
            ...
            if row['date_timestamp'].time() >= time(15,0,0):
                ...
                if put['pos'] == -1:
                    if not gen_put[(gen_put['date_timestamp'] == dt) & (gen_put['strike'] == put['strike'])].empty:
                        Put_Option = gen_put[(gen_put['date_timestamp'] == dt) & (gen_put['strike'] == put['strike'])].iloc[0]
                        capital, rollover = buy(Put_Option, buying, io,id, put, row, "PE", capital, rollover, initial_capital)
                        execs.append({
                        ...
                        put.update({'pos': 0, 'movement': [], 'sharpie_ratio': 0, 'drawdown': 0, 'entry_time': None,'strike': None, 'entry_price': 0.0, 'exit_time': None, 'capital': initial_capital, 'rollover': 0,'exit_price': None, 'pnl': 0, 'type': None, 'pnl_sum': 0, 'transaction_cost': 0.0, 'slippage': 0, 'logs': [],'stop_loss': 0, 'profit_cap': 0, 'entry_delta': 0.0,'exit_delta':0.0})
                    else:
                ...
                if call['pos'] == -1:
                    if not gen_call[(gen_call['date_timestamp'] == dt) & (gen_call['strike'] == call['strike'])].empty:
                        ...
                        Call_Option = gen_call[(gen_call['date_timestamp'] == dt) & (gen_call['strike'] == call['strike'])].iloc[0]
                        capital, rollover = buy(Call_Option, buying, io,id, call, row, "CE", capital, rollover,initial_capital)
                        execs.append({
                        ...
                        call.update({'pos': 0, 'movement': [], 'sharpie_ratio': 0, 'drawdown': 0, 'entry_time': None,'strike': None, 'entry_price': 0, 'exit_time': None, 'capital': initial_capital, 'rollover': 0,'exit_price': None, 'pnl': 0, 'type': None, 'pnl_sum': 0.0, 'transaction_cost': 0, 'slippage': 0, 'logs': [],'stop_loss': 0, 'profit_cap': 0, 'entry_delta': 0.0,'exit_delta':0.0})
                    else:
                        print(f'didnt able to find call with strike {call["strike"]} at dt = {dt}',file=file)

            if put['pos'] != 0 and call['pos'] != 0:
                Call_Sq_Off = call['movement'][0] - call['movement'][-1] + (call['movement'][-1] + call['movement'][0]) * (215 / 100000)
                Put_Sq_Off = put['movement'][0] - put['movement'][-1] + (put['movement'][-1] + put['movement'][0] ) * (215 / 100000)
    ...
    if put['pos'] == -1 or call['pos'] == -1:
        if put['pos']==-1:
            Put_Option = gen_put[(gen_put['strike'] == put['strike'])].sort_values(by='date_timestamp',ascending=False).iloc[0]
            capital, rollover = buy(Put_Option, buying, io,id, put, futures.iloc[-1], "PE", capital, rollover, initial_capital)
            execs.append({
                'id': id, 'time': Put_Option['date_timestamp'], 'opt_type': 'PE', 'log': f"Final Sq_off", 'delta': Put_Option['delta'],
        ...
        if call['pos']==-1:
            Call_Option = gen_call[(gen_call['strike'] == call['strike'])].sort_values(by='date_timestamp',ascending=False).iloc[0]
            capital, rollover = buy(Call_Option, buying, io,id, call, futures.iloc[-1], "CE", capital, rollover,initial_capital)
            execs.append({
                'id': id, 'time': Call_Option['date_timestamp'], 'opt_type': 'CE', 'log': f"Final Sq_off", 'delta': Call_Option['delta'],
...
def buy(option, buying, io,id, obj, row, type, capital, rollover, initial_capital):
    if capital <= option[buying]:
        ...
        obj.update({'pos': 0, 'exit_time': row['date_timestamp'], 'exit_price': option[buying],'exit_delta':option['delta'],
            ...
            'pnl': round(obj['entry_price']-option[buying] - (obj['entry_price'] + option[buying]) * (115 / 100000) - (obj['entry_price'] + option[buying]) * (1 / 1000),3),
            'type': type})
